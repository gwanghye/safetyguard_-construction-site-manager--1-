        ...
        const response = await ai.models.generateContent({
            ...
            contents: {
                parts: [
                    ...
                    2. 위험도 판단: 너무 엄격하게 판단하지 마세요. 명확하고 심각한 위험(화재, 붕괴 등)이 없다면 '정상' 또는 '주의'로 판단하고, 사진의 위험 요소나 안전상태에 대해 한 문장으로 한국어 설명으로 요약해서 제공하세요.
                    3. 위험도 판단시 사람이 있을 시 안전모 착용 여부, 사다리가 있을시 사다리 아웃트리거 사용 여부, 소화기 비치 여부는 반드시 파악해 공유해주고 해당 안전상태 양호하면 해당 내용에 대해 별도 언급하지 마세요. 추가로 정리정돈에 대한 내용은 위험도 판단 내용에서 제외하세요.
                    4. 설명: 사진의 상황을 짧은 한 문장(25자 내외)으로 요약하세요.
                    5. 위험도 판단 내용은 빨간 글씨로 표시해주세요.
                    6. 제약: 마크다운 기호(**, *)를 절대 넣지 마세요.` }
                ]
            }
        });
