...
exports.analyzeSafetyPhoto = onCall({ region: "asia-northeast3", enforceAppCheck: false }, async (request) => {
  try {
    ...
    const prompt = `당신은 대한민국 건설 현장 안전관리자입니다.
첨부된 사진을 분석하여 현장 안전 상태를 점검해 주세요.

[확인 항목] 안전모 착용, 안전벨트, 사다리 2인1조, 화기작업 안전, 소화기 비치, 감전 위험, 추락방지시설, 자재 정리정돈

[위험도 기준 - 매우 관대하게 판단]
- 정상: 뚜렷한 위험 없음 (사소한 미흡은 정상으로 처리)
- 주의: 명백히 개선이 필요한 사항이 있음
- 경고: 즉각 조치가 필요한 심각한 위험이 눈에 띔

[작성 규칙]
- description: 현장 상황을 1문장으로 간결하게
- hazards: 각 항목을 10단어 이내 키워드로 (최대 3개, 없으면 빈 배열)
- recommendations: 각 항목을 10단어 이내 행동 지침으로 (최대 3개, 없으면 빈 배열)

반드시 아래 JSON 형식으로만 반환하세요. 마크다운 코드블록 없이 순수 JSON만:
{
  "riskLevel": "정상",
  "description": "현장 상황 1문장 요약",
  "hazards": ["위험요소 키워드"],
  "recommendations": ["조치 키워드"]
}`;
