...
첨부된 사진을 분석하여 다음 사항을 파악해 주세요.
(1) 사진 속의 장소 및 상황 설명
(2) 잠재적인 위험 요소 (추락, 화재, 감전, 협착 등)
(3) 해당 위험 요소를 해결하기 위한 구체적인 조치 방법
(4) 전반적인 위험도 (양호, 주의, 위험 중 택 1)
...
반드시 아래 JSON 형식으로만 답변을 반환하세요. 마크다운 기호 없이 순수 JSON 문자열만 출력해야 합니다.
{
  ...
  "riskLevel": "주의"
    ...
    const response = await ai.models.generateContent({
      ...
      contents: [
        { role: 'user', parts: [ { text: prompt }, { inlineData: { data: imageBase64, mimeType: mimeType } } ] }
      ],
      config: {
